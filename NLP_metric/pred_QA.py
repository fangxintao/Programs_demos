...
def pred_one(input_str_ls):
    ...
    for output in output_ls:
        try:
            # output_tokens = voc.decode(output[1:-1])
            output_tokens = sp.Decode(output[1:].tolist())
        except:
            print('error', output)
            continue
        outputs.append(output_tokens.strip())
...
def print_acc_accuracy(infile, outfile):
...
if __name__ == '__main__':
    ...
    for dataset_name in list_dataset:
        ...
        with open(os.path.join(checkpoint_path, f'pred_QA_scifive.csv'), 'a', newline='') as csvfile:
